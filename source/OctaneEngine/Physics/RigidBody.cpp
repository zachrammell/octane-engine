...
void RigidBody::Integrate(float dt)
{
  //integrate velocity
  //calculate linear
  linear_velocity_ = DirectX::XMVectorAdd(linear_velocity_, DirectX::XMVectorScale(force_accumulator_, mass_data_.inverse_mass * dt));
  //calculate angular
  angular_velocity_ = DirectX::XMVectorAdd(angular_velocity_, DirectX::XMVectorScale(XMVector3Transform(torque_accumulator_, global_inverse_inertia_), dt));
  //reset accumulator.
  force_accumulator_  = XMLoadFloat3(&zero_vector_);
  torque_accumulator_ = XMLoadFloat3(&zero_vector_);
  //integrate transformation
  //calculate linear
  DirectX::XMVECTOR delta_linear_velocity = HadamardProduct(DirectX::XMVectorScale(linear_velocity_, dt), linear_constraints_);
  global_centroid_                        = DirectX::XMVectorAdd(global_centroid_, delta_linear_velocity);
  //calculate angular
  DirectX::XMVECTOR delta_angular_velocity = HadamardProduct(DirectX::XMVectorScale(angular_velocity_, dt), angular_constraints_);
...
void RigidBody::ApplyForce(const DirectX::XMFLOAT3& force, const DirectX::XMFLOAT3& at)
{
  ...
  torque_accumulator_         = DirectX::XMVectorAdd(torque_accumulator_, DirectX::XMVector3Cross(DirectX::XMVectorSubtract(XMLoadFloat3(&at), global_centroid_), force_vec));
}
...
{
  global_centroid_ = DirectX::XMVectorAdd(Rotate(inverse_orientation_, orientation_, mass_data_.local_centroid), position_);
}

void RigidBody::UpdatePosition()
{
  position_ = DirectX::XMVectorAdd(Rotate(inverse_orientation_, orientation_, DirectX::XMVectorNegate(mass_data_.local_centroid)), global_centroid_);
}
...
void RigidBody::UpdateInertia()
{
  ...
  global_inverse_inertia_               = XMMatrixMultiply(XMMatrixMultiply(orientation, mass_data_.local_inverse_inertia), inverse_orientation);
