...
cbuffer cb_per_frame : register(b1)
{
  ...
  float4 CameraPosition; //assumes weapons will look good with every camera
...
struct vs_out
{
  float4 position_ndc : SV_POSITION; // position in NDC coordinates
  float4 position_world : WPOSITION; //world position
  float4 uv : TEXCOORD;
...
{
  vs_out output;
  output.position_world = CameraPosition;//mul(World, float4(input.position_local, 1.0f));
